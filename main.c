...
void printFileInfo(struct dirent *file, int mode, int size_type) {
	...
	switch (eStat.st_mode & S_IFMT) {
		case S_IFBLK: sprintf(file_perm, "%s%c", file_perm, 'b'); break;
		case S_IFCHR: sprintf(file_perm, "%s%c", file_perm, 'c'); break;
		case S_IFDIR: sprintf(file_perm, "%s%c", file_perm, 'd'); break;
		case S_IFIFO: sprintf(file_perm, "%s%c", file_perm, 'p'); break;
		case S_IFLNK: sprintf(file_perm, "%s%c", file_perm, 'l'); break;
		case S_IFREG: sprintf(file_perm, "%s%c", file_perm, '-'); break;
		case S_IFSOCK: sprintf(file_perm, "%s%c", file_perm, 's'); break;
		default: sprintf(file_perm, "%s%c", file_perm, ' '); break;
	}
	...
	if (mode)
		printf("%s %s %s %15s %s %s\n", file_perm, pws->pw_name,
		       file_group->gr_name, file_size_str, time, file->d_name);
	else
...
int main(int argc, char **argv) {
	...
	if (n >= 0) {
		...
		if (!rev) {
			for (int i = 0; i < n; ++i) {
				if (!all & files[i]->d_name[0] == '.') continue;
